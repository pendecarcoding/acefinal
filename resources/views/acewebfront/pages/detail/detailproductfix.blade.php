...
<main>
@php
$tmp_user = session()->get('temp_user_id');
$totalcart = ($tmp_user != null) ? \App\Models\Cart::where('temp_user_id',$tmp_user)->count() : 0;
@endphp
                        ...
                        <div class="text-left">
                            ...
                            <div class="row no-gutters mt-3">
                                <div class="col-auto">
                                    ...
                                    @else
                                        {{ translate('Ace Inovate Asia Berhad') }}
                                    @endif
                                </div>
                                    ...
                                    @if (Auth::check() && addon_is_activated('affiliate_system') && (\App\Models\AffiliateOption::where('type', 'product_sharing')->first()->status || \App\Models\AffiliateOption::where('type', 'category_wise_affiliate')->first()->status) && Auth::user()->affiliate_user != null && Auth::user()->affiliate_user->status)
                                        @php
                                            if (Auth::check()) {
                                                ...
                                                $referral_code_url = URL::to('/our_products') . '/' . $detailedProduct->slug . "?product_referral_code=$referral_code";
                                            }
                                        @endphp
                                        <div>
