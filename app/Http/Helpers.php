...
function updatecountview($id){
    try {
        $last = Blog::where('id',$id)->first();
        $now = $last->show + 1;
        $data = [
            'show'=>$now
        ];
        $act = Blog::where('id',$id)->update($data);
...
function getsignm1payment($sign){
    $signature = '';
    $data      = $sign;
    //$data    = "Just try|322.95|opsuuduue1|MYR|MYR|bohati@silverstream.my|41012618";
    //return env('M1_CLIENT_SECRET');
    try {
        $priv_key = env('M1_PRIVATE_KEY');
        $pkeyid   = openssl_get_privatekey($priv_key);
        openssl_sign($data, $signature, $pkeyid, "sha1WithRSAEncryption");
        return strToHex($signature);
       // return env('M1_CLIENT_SECRET');
    } catch (Exception $e) {
...
function gettokenm1payment($url){

    /*$postRequest = array(
    'grant_type'=>'client_credentials',
    'client_id'=>env('M1_CLIENT_ID'),
    'client_secret'=>env('M1_CLIENT_SECRET')
);*/
$client_id     = env('M1_CLIENT_ID');
$client_secret = env('M1_CLIENT_SECRET');
$postRequest = "grant_type=client_credentials&client_id=$client_id&client_secret=$client_secret";
...
function getnav(){
    $data = array(
        ...
        'name'=>'Our Products',
        'link'=>'our_products',
        'is_active'=>'our_products'
         ],[
        'name'=>'Newsroom',
        'link'=>'newsroom',
        'is_active'=>['newsroom','news*']
         ],[
        'name'=>'Contact',
        'link'=>'contact',
        'is_active'=>'contact'
...
if (!function_exists('format_price')) {
    function format_priceback($price, $isMinimize = false)
    {
...
if (!function_exists('cart_product_price_base')) {
    function cart_product_price_base($cart_product, $product, $formatted = true, $tax = true)
    {
...
function getShippingCost($carts, $index, $carrier = '')
{
    ...
    if ($shipping_type == 'flat_rate') {
        ...
        $shipping_info = Address::where('temp_id', $carts[0]['temp_user_id'])->first();
